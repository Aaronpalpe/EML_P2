...
de \textit{pruning} y \textit{quantization}.
...
de compresión específico para tensores \textit{sparse} que permita aprovechar el \textit{pruning}, por lo que al final se está mandando la misma
...
usando CPU, por lo que la cuantización BF16 solo se estaba simulando y no obteníamos mejoras en tiempo. Esto se debía a que en la función \texttt{get\_available\_gpu} de \texttt{controller.py} solo se detectaba una GPU si 
